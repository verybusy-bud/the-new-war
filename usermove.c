...
void piece_move(piece_info_t *obj) {
	...
	while (obj->moved < obj_moves(obj)) {
		...
		if (obj->type == FIGHTER && obj->hits > 0) {
			...
			/* range can be less than zero if fighter survives many
			   attacks. It loses 1 unit of range on every attack it
			   survives. */
			else if (obj->range <= 0) {
...
void move_armyload(piece_info_t *obj) {
	...
	if (loc != obj->loc) {
		...
		int i;
		(void)memcpy(amap, game.user_map,
...
void user_dir_army(piece_info_t *obj, loc_t loc) {
	...
	if (game.user_map[loc].contents == 'O' && 
		...
		if (game.user_map[obj->loc].contents == 'T') {
			...
			comment("Your army marched dutifully into the sea and "
			        "drowned.");
			...
			{
				comment("Your army regretfully drowns after "
				        "its successful assault.");
				ksend("Your army regretfully drowns after its "
				      "successful assault.");
			}
		}
		if (obj->hits > 0) {
...
void user_dir_ship(piece_info_t *obj, loc_t loc) {
	if (game.real_map[loc].contents == MAP_CITY) {
		...
		if (game.user_map[loc].contents == MAP_LAND) {
			...
			if (obj->hits > 0) /* ship won? */
			{
				comment("Your %s breaks up after its "
				        "successful assault.",
				        piece_attr[obj->type].name);
				ksend("Your %s breaks up after its successful "
				      "assault.",
				      piece_attr[obj->type].name);
			}
		}
