...
int getint(char *message) {
	...
	for (;;) { /* until we get a legal number */
		...
		for (p = buf; *p; p++) {
			if (*p < '0' || *p > '9') {
				error("Please enter an integer.", 0, 0, 0, 0, 0,
				      0, 0, 0);
				break;
			}
		}
		if (*p == 0) { /* no error yet? */
			if (p - buf > 7) /* too many digits? */ {
				error("Please enter a small integer.", 0, 0, 0,
				      0, 0, 0, 0, 0);
			} else {
				return (atoi(buf));
			}
...
int get_range(char *message, int low, int high) {
	for (;;) {
		...
		error("Please enter an integer in the range %d..%d.", low,
		      high);
	}
}
...
void help(char **text, int nlines) {
	...
	pos_str(text_lines + NUMTOPS + 2, 1,
